package logic 
{
	 ...
	 * @author Yun
	 */
	public class XitoLogic 
	{
		public var groupName:Array = ["", "Thùng phá sảnh", "Tứ quý", "Cù lũ", "Thùng", "Sảnh", "Sám cô", "Thú", "Độn", "Mậu thầu"];
		private static var chipValueArray:Array = [100000000000, 10000000000, 1000000000, 100000000, 10000000, 1000000, 100000, 10000, 1000];
		public function XitoLogic() 
		{
			
		}
		
		private static var _instance:XitoLogic;
		static private var mainData:MainData = MainData.getInstance();
		public static function getInstance():XitoLogic
		{
			if (!_instance)
				_instance = new XitoLogic();
			return _instance;
		}
		
		public static function convertMoneyToChip(money:Number):Array
		{
			var tempMoney:Number = money;
			var chipArray:Array = new Array();
			var tempClass:Class;
			for (var i:int = 0; i < chipValueArray.length ; i++) 
			{
				var tempArray:Array = new Array();
				if (tempMoney >= chipValueArray[i])
				{
					var tempNumber:int = int(tempMoney / chipValueArray[i]);
					for (var j:int = 0; j < tempNumber; j++) 
					{
						switch (chipValueArray[i]) 
						{
							case 100000000000:
								tempArray.push(new Chip100B());
							break;
							case 10000000000:
								tempArray.push(new Chip10B());
							break;
							case 1000000000:
								tempArray.push(new Chip1B());
							break;
							case 100000000:
								tempArray.push(new Chip100M());
							break;
							case 10000000:
								tempArray.push(new Chip10M());
							break;
							case 1000000:
								tempArray.push(new Chip1M());
							break;
							case 100000:
								tempArray.push(new Chip100K());
							break;
							case 10000:
								tempArray.push(new Chip10K());
							break;
							case 1000:
								tempArray.push(new Chip1K());
							break;
							default:
						}
					}
					tempMoney -= int(tempMoney / chipValueArray[i]) * chipValueArray[i];
				}
				if (tempArray.length > 0)
					chipArray.push(tempArray);
				if (chipArray.length >= 4)
					return chipArray;
			}
			return chipArray;
		}
		...
		public function checkGroup(type:int, cards:Array):int
		{
			...
			{
				tempArray.push(CardXito(cards[i]).id);
			}
		...
		private function deckOneStrongerDeckTwo(deckOneIndex:int, deckTwoIndex:int, cards:Array, deckRank:Array):Boolean 
		{
			...
			if (deckOneIndex == 3) // Nếu so sánh chi 3 với chi 2
			{
				for (i = 0; i < 3; i++) 
				{
					deck1.push(CardXito(cards[i]).id);
				}
				for (i = 3; i < 8; i++) 
				{
					deck2.push(CardXito(cards[i]).id);
				}
		...
		public function check3Thung(cards:Array):Boolean
		{
			...
			{
				suitArray.push(convertIdToSuite(CardXito(cards[i]).id));
			}
		...
		public function check3Sanh(cards:Array):Boolean
		{
			...
			for (var i:int = 0; i < 3; i++) 
			{
				rankArray.push(convertIdToRank(CardXito(cards[i]).id));
			}
			rankArray.sort(Array.NUMERIC);
		...
		public function arrangeAllCard(cards:Array, isIncrease:Boolean = true):void
		{
			...
			while (!arrangeFinish)
			{
				...
				for (var i:int = 0; i < cards.length - 1; i++) 
				{
					if (isIncrease)
					{
						trace(convertIdToRank(CardXito(cards[i]).id),convertIdToRank(CardXito(cards[i + 1]).id));
						if (convertIdToRank(CardXito(cards[i]).id) > convertIdToRank(CardXito(cards[i + 1]).id))
						{
							trace("swap");
							var tempCard:CardXito = cards[i];
							cards[i] = cards[i + 1];
							cards[i + 1] = tempCard;
							arrangeFinish = false;
						}
					...
					{
						if (CardXito(cards[i]).id > CardXito(cards[i + 1]).id)
						{
							tempCard = cards[i];
							cards[i] = cards[i + 1];
							cards[i + 1] = tempCard;
