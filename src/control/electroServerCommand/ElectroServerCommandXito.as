package control.electroServerCommand 
{
	...
	public class ElectroServerCommandXito 
	{
		...
		public function ElectroServerCommandXito() 
		{
		...
		public function startConnect(_userName:String, _channelId:int, _capacity:int = -1, _pass:String = ""):void
		{
			...
			if (!coreAPI)
				coreAPI = new CoreAPIXito(configuration);
		...
		{
			mainData.publicChatData.userName = e.data[DataFieldXito.USER_NAME];
			mainData.publicChatData.displayName = e.data[DataFieldXito.DISPLAY_NAME];
			mainData.publicChatData.chatContent = e.data[DataFieldXito.CHAT_CONTENT];
		...
		private function onServerConfirmAddFriendInvite(e:ElectroServerEvent):void 
		{
			...
			var invitedNameArray:Array = [e.data[DataFieldXito.FRIEND_ID]];
		...
		private function onServerConfirmAddFriendConfirm(e:ElectroServerEvent):void 
		{
			...
			esObject.setString(DataFieldXito.USER_NAME, mainData.chooseChannelData.myInfo.uId);
			esObject.setString(DataFieldXito.MESSAGE, mess);
			esObject.setBoolean(DataFieldXito.SUCCESS, e.data[DataFieldXito.SUCCESS]);
		...
		{
			if (!infoObject[DataFieldXito.ROOM_PASSWORD])
				infoObject[DataFieldXito.ROOM_PASSWORD] = "";
		...
		private function onUpdateRoomList(e:ElectroServerEvent):void 
		{
			...
			for (var roomId:String in roomList)
			{
				...
				if(roomList[roomId][DataFieldXito.IS_SEND_CARD])
				...
				roomData.male = roomList[roomId][DataFieldXito.MALE];
				roomData.betting = roomList[roomId][DataFieldXito.ROOM_BET];
				roomData.channelId = mainData.playingData.gameRoomData.channelId;
				roomData.hasPassword = roomList[roomId][DataFieldXito.HAS_PASSWORD];
				roomData.maxPlayer = roomList[roomId][DataFieldXito.MAX_PLAYER];
				roomData.name = roomList[roomId][DataFieldXito.ROOM_NAME];
				roomData.id = int(roomId);
				roomData.gameId = roomList[roomId][DataFieldXito.GAME_ID];
				...
				roomData.userNumbers = roomList[roomId][DataFieldXito.USERS_NUMBER];
		...
		private function onRemoveFriend(e:ElectroServerEvent):void 
		{
			if(coreAPI.myData.friendList)
				delete coreAPI.myData.friendList[e.data[DataFieldXito.USER_NAME]];
		...
		private function onFriendConfirmAddFriendInvite(e:ElectroServerEvent):void 
		{
			...
			if (coreAPI.myData.friendList)
			{
				...
				{
					coreAPI.myData.friendList[e.data[DataFieldXito.USER_NAME]] = new Object();
					coreAPI.myData.friendList[e.data[DataFieldXito.USER_NAME]][DataFieldXito.DISPLAY_NAME] = e.data[DataFieldXito.DISPLAY_NAME];
				}
			}
		}
		...
		private function onUpdateUserList(e:ElectroServerEvent):void 
		{
			...
			for (var userName:String in allUserList)
			{
				...
				if (allUserList[userName][DataFieldXito.USER_INFO])
				{
					userData.roomID = allUserList[userName][DataFieldXito.ROOM_ID];
					userData.moneyLogoUrl = allUserList[userName][DataFieldXito.USER_INFO][DataFieldXito.LOGO];
					userData.displayName = allUserList[userName][DataFieldXito.USER_INFO][ModelField.DISPLAY_NAME];
					userData.win = allUserList[userName][DataFieldXito.USER_INFO][DataFieldXito.WIN];
					userData.lose = allUserList[userName][DataFieldXito.USER_INFO][DataFieldXito.LOSE];
					userData.levelName = allUserList[userName][DataFieldXito.USER_INFO].level;
					userData.userID = userName;
					userData.userName = userName;
					userData.money = allUserList[userName][DataFieldXito.USER_INFO][ModelField.MONEY];
					
					userData.avatar = allUserList[userName][DataFieldXito.USER_INFO][ModelField.AVATAR];
					...
					if (allUserList[userName][DataFieldXito.USER_INFO][DataFieldXito.LOGO])
						userData.webLogoUrl = allUserList[userName][DataFieldXito.USER_INFO][DataFieldXito.LOGO];
					else
						userData.webLogoUrl = '';
				}
				else
				...
				if (allUserList[userName][DataFieldXito.ROOM_ID] == mainData.lobbyRoomId)
				{
					userData.isJoinRoom = false;
				...
				{
					userData.description = "Phòng " + allUserList[userName][DataFieldXito.ROOM_ID];
				}
