package view.card 
{
	 ...
	 * @author Yun
	 */
	public class CardMauBinh extends Sprite 
	{
		...
		public function CardMauBinh(_size:Number = 1)
		{
			size = _size;
			
			scaleX = scaleY = size;
			
			addContent("zCardMauBinh");
		...
		public function moving(finishPoint:Point, movingTime:Number, movingType:String, scaleNumber:Number = 0.88, rotationNumber:Number = 0,reAddChild:Boolean = true,reAddStartPoint:Boolean = true,reRotate:Boolean = true, alphaNumber:Number = 1, isBackOut:Boolean = true):void
		{	
			if (movingTween)
				movingTween.end();
			...
			isMoving = true;
			if(reRotate)
				this.rotation = rotationNumber + 100;
			switch (movingType) 
			{
				case CardManagerMauBinh.TURN_OVER_STYLE:
					if(isBackOut)
				...
				break;
				case CardManagerMauBinh.OPEN_MIDDLE_STYLE:
					movingTween = new GTween(this, movingTime, { x:tempPoint.x, y:tempPoint.y, scaleX:scaleNumber, scaleY:scaleNumber, rotation:rotationNumber, alpha:alphaNumber }, { ease:Back.easeOut });
					effectOpen(movingTime);
					movingTween.addEventListener(Event.COMPLETE, movingComplete);
				break;
				case CardManagerMauBinh.OPEN_FINISH_STYLE:
					movingTween = new GTween(this, movingTime, { x:tempPoint.x, y:tempPoint.y, scaleX:scaleNumber, scaleY:scaleNumber, rotation:rotationNumber, alpha:alphaNumber }, { ease:Back.easeOut });
		...
		private function onMouseClick(e:MouseEvent):void 
		{
			...
			if (isChoose)
			{
				...
				moving(point, CardManagerMauBinh.clickCardTime, CardManagerMauBinh.TURN_OVER_STYLE, 0.88, 0, false, false);
		...
		public function moveUp(size:Number):void
		{
			if (movingTween)
				movingTween.end();
			...
			point.x = startX
			point.y = startY - height / 3;
			point = parent.localToGlobal(point);
			moving(point, CardManagerMauBinh.clickCardTime, CardManagerMauBinh.TURN_OVER_STYLE, size, 0, false, false, false, 1, true);
		}
		
		public function moveDown(size:Number):void
		{
			if (movingTween)
				movingTween.end();
			...
			point.y = startY;
			point = parent.localToGlobal(point);
			moving(point, CardManagerMauBinh.clickCardTime, CardManagerMauBinh.TURN_OVER_STYLE, size, 0, false, false, false);
		}
		
		private var _isArrange:Boolean; // Đã được chuyển vào vị trí sắp xếp
