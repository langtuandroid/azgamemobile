package view.card 
{
	 ...
	 * @author Yun
	 */
	public class CardManagerTLMN extends Sprite 
	{
		...
		public static const NO_EFFECT:String = "noEffect"; // không có effect
		...
		public function CardManagerTLMN() 
		{
		...
		{
			var player:PlayerInfo = playerArray[countDivideIndex];
			...
			if (player)
			{
				...
				if (player.cardInfoArray.length == 0)
				{
					if (player.formName == PlayerInfo.BELOW_USER)
					{
			...
			{
				if (PlayerInfo(playerArray[i]).cardInfoArray.length == 0)
					countDivideFinish++;
			}
			...
			if (countDivideFinish == playerArray.length)
			{
				if (timerToDivide)
				{
					...
					timerToDivide.stop();
					timerToDivide = null;
					getCardPoint.visible = false;
				}
			}
		}
		
		public function divideOneCard(player:PlayerInfo, cardId:int, time:Number = 0):void
		{
			if (time == 0)
				time = cardToDesTime;
				
			var tempCard:Card = new Card(0.5);
		 ...
		 */
		public function playCard(player:PlayerInfo,cardValues:Array):void
		{
			player.playCard(cardValues);
		}
		 ...
		 */
		public function addValueForOneUnleavedCard(player:PlayerInfo, cardId:int):void
		{
			player.addValueForOneUnleavedCard(cardId);
		}
		
		public function addAllCard(player:PlayerInfo, cardInfo:Object):void
		{
			if (cardInfo[DataField.GAME_STATE] != DataField.WAITING)
			{
				...
				{
					card = new Card(player.unLeaveCardSize);
					card.id = 0;
		...
		private function getPointByCardType(player:PlayerInfo,cardType:String):Point 
		{
